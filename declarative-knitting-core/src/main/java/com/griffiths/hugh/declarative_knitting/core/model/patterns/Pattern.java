...
public class Pattern {
	...
	public Pattern addColour(int index, Color colour){
		colours.put(index, colour);

		return this;
	}

	public int createColour(Color colour){
		Optional<Integer> max = colours.keySet().stream().max(Comparator.naturalOrder());
		final int nextInt;
		if (max.isPresent())
			nextInt=max.get()+1;
		else
	...
	public PatternSegment createSegment(String title, int castOnStitchs){
		PatternSegment newSegment = PatternSegment.castOn(castOnStitchs);
